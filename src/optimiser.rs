...
// simple naive brainfuck optimisations, TODO: make this more complex?
pub struct BrainfuckOptimiser;

impl BrainfuckOptimiser {
...
mod tests {
	...
	fn subset_equivalence_test_0() {
		...
		let o: String = BrainfuckOptimiser::optimise_subset(v.chars().collect())
			.into_iter()
			.collect();
		assert_eq!(o, ">>+++<---<+++++<<<+++");
	}
	...
	fn program_equivalence_test_0() {
		let v = String::from("<><><>++<+[--++>>+<<-]");
		let o: String = BrainfuckOptimiser::optimise(v.chars().collect());
		assert_eq!(o, "++<+[->>+<<]");
	}
