...
import brainfuck from "../../docs/brainfuck.md?raw";
...
const DocsModal: Component<{ style?: JSX.CSSProperties }> = () => {
  ...
  const [docsContent, setDocsContent] = createSignal(
    docs[selected() as keyof typeof docs] ?? ""
  );
  createEffect(() => {
  ...
  function prevDoc() {
    setSelected(
      titles[
        (titles.indexOf(selected() ?? "") - 1 + titles.length) % titles.length
      ]
    );
  }

  return (
    <Show when={app.docsOpen()}>
