...
use crate::{
	...
	parser::{
		Clause, Expression, ExtendedOpcode, VariableDefinition, VariableTarget, VariableType,
	},
	MastermindConfig,
};
...
// this is an enum to support multiple types of struct (like rust has tuple structs() or named structs{})
enum Struct {
	NamedFields(HashMap<String, VariableType>),
}
...
impl Scope<'_> {
	...
	fn allocate_variable_memory(
		...
		let memory = match &var.var_type {
			...
			VariableType::Struct(_) | VariableType::Array(_, _) => Memory::Cells {
				id,
				len: self.size_of_type(&var.var_type)?,
			},
		};
	...
	/// return the variable type size in bytes, dependent on scope context for struct type definitions
	fn size_of_type(&self, var_type: &VariableType) -> Result<usize, String> {
		Ok(match var_type {
			VariableType::Cell => 1,
