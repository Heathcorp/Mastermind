...
pub fn parse<TC: TapeCellLocation, OC: OpcodeVariant>(
	tokens: &[Token],
...
fn parse_let_clause<TC: TapeCellLocation, O>(clause: &[Token]) -> Result<Clause<TC, O>, String> {
	// cell x = 0;
...
fn parse_struct_clause<TC: TapeCellLocation, O>(clause: &[Token]) -> Result<Clause<TC, O>, String> {
	let mut i = 0usize;
...
fn parse_set_clause<T, O>(clause: &[Token]) -> Result<Vec<Clause<T, O>>, String> {
...
impl TapeCellLocation for TapeCell {
	fn parse_location_specifier(
		tokens: &[Token],
	) -> Result<(LocationSpecifier<TapeCell>, usize), String> {
		if tokens.len() == 0 {
			return Ok((LocationSpecifier::None, 0));
		}
		let Token::At = &tokens[0] else {
			return Ok((LocationSpecifier::None, 0));
		};
...
fn parse_var_definition<TC: TapeCellLocation>(
	tokens: &[Token],
	allow_location: bool,
) -> Result<(VariableDefinition<TC>, usize), String> {
	let mut i = 0usize;
	...
	let (location_specifier, len) = TC::parse_location_specifier(&tokens[i..])?;
...
#[cfg(test)]
mod parser_tests {
	use crate::backend::{bf::TapeCell, bf2d::Opcode2D};
